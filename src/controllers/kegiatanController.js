...
const getDetailByPO = async (req, res) => {
    ...
    try {
        ...
        // 🔥 PERBAIKAN DI SINI: Tambahkan t.id as transporter_id
...
const getTrukByAlokasi = async (req, res) => {
    ...
    try {
        const query = `
            SELECT k.id, k.plat_nomor 
            FROM kendaraan k
            JOIN kegiatan_kendaraan kk ON k.id = kk.kendaraan_id
            JOIN kegiatan_transporter kt ON kk.kegiatan_transporter_id = kt.id
            WHERE kt.kegiatan_id = ? AND kt.transporter_id = ?
            ORDER BY k.plat_nomor ASC
        `;
        const [rows] = await db.query(query, [kegiatan_id, transporter_id]);
        res.json(rows);
    } catch (error) {
        console.error("❌ ERROR GET TRUK ALOKASI:", error);
        res.status(500).json({ message: error.message });
    }
};
