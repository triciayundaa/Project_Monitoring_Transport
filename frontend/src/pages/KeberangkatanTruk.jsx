...
const KeberangkatanTruk = () => {
    ...
    // --- STATE BARU: LIST TRANSPORTER ---
    const [transporterList, setTransporterList] = useState([]);
    // ------------------------------------

    const [formData, setFormData] = useState({
        transporter_id: '', // Tambahan field
        no_polisi: '',
        no_seri_pengantar: '',
        foto_truk: null,
        foto_surat: null
    ...
    const handleInputDataBaru = () => {
        ...
        // Time validation HANYA untuk tanggal hari ini
        if (selectedDate === todayString) {
            const now = new Date();
    ...
    const handleCekPO = async () => {
        ...
        try {
            ...
            if (response.data.status === 'Success') {
                ...
                setPoData(dataPO);
                // SIMPAN LIST TRANSPORTER DARI BACKEND
    ...
    const handleCapturePhoto = async (field) => {
        try {
            ...
            video.addEventListener('loadedmetadata', () => { 
                // Kompresi agresif: max 600px, quality 0.15
                const maxWidth = 600;
            ...
            modal.querySelector('#capture-btn').addEventListener('click', () => {
                ...
                const imageData = canvas.toDataURL('image/jpeg', 0.15); // Kompresi agresif (15%)
    ...
    const confirmHapus = async () => {
        ...
        try {
            const response = await axios.delete(`http://localhost:3000/api/keberangkatan/${deleteConfirmId}`);
            if (response.data.status === 'Success') { 
                setTruckDepartures(prev => prev.filter(item => item.id !== deleteConfirmId));
                setShowModalConfirmDelete(false);
                setDeleteConfirmId(null);
            }
        } catch (error) { 
            setAlertMessage('Gagal hapus data');
            setShowModalAlert(true);
            setShowModalConfirmDelete(false);
            setDeleteConfirmId(null);
        }
                ...
                <div className="fixed inset-0 bg-black bg-opacity-70 z-[70] flex items-center justify-center p-2 sm:p-4 overflow-hidden">
                    <div className="bg-white rounded-lg w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl relative">
                        ...
                        <div className="bg-white border-t p-4 flex justify-between items-center text-sm">
                            <button onClick={() => setCurrentMonthInfo(prev => { if (prev.month === 0) return { month: 11, year: prev.year - 1 }; return { ...prev, month: prev.month - 1 }; })} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded text-gray-700 font-medium">← Bulan Sebelumnya</button>
                            <span className="font-bold text-gray-700 hidden sm:block">Navigasi Jadwal</span>
                            <button onClick={() => setCurrentMonthInfo(prev => { if (prev.month === 11) return { month: 0, year: prev.year + 1 }; return { ...prev, month: prev.month + 1 }; })} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded text-gray-700 font-medium">Bulan Berikutnya →</button>
                        </div>
                    </div>
                </div>
            )}
                            ...
                            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        {formData.foto_truk && (<div className="mt-2"><img src={formData.foto_truk} alt="Foto Truk" className="w-full h-48 object-cover rounded-lg" /><button type="button" onClick={() => setFormData(prev => ({ ...prev, foto_truk: null }))} className="mt-2 text-sm text-red-600 hover:text-red-700">Hapus Foto</button></div>)}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        {formData.foto_surat && (<div className="mt-2"><img src={formData.foto_surat} alt="Foto Surat Pengantar" className="w-full h-48 object-cover rounded-lg" /><button type="button" onClick={() => setFormData(prev => ({ ...prev, foto_surat: null }))} className="mt-2 text-sm text-red-600 hover:text-red-700">Hapus Foto</button></div>)}
                                    </div>
                                </div>
